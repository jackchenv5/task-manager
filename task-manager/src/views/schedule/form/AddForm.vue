...
const { curReceivers, curSelectProjectRef, curSelectDateStat } = storeToRefs(
  scheduleStore
);
...
const onSubmit = async (formRef) => {
    ...
    const tmpFormData = {
      ...
      start_time:
        formData.start_time instanceof Date
          ...
          : formData.start_time,
      deadline_time:
        formData.deadline_time instanceof Date
          ? formatDate(formData.deadline_time)
  ...
  VxeUI.modal.message({
    content: `任务：${formData.name},执行者：${formData.receiver
      .map((x) => x.username)
      .join(",")} 已添加！`,
    status: "success",
  });
